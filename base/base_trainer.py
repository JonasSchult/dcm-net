...
class BaseTrainer:
    ...
    """
    def __init__(self, model, loss, optimizer, lr_scheduler, resume, config, train_logger=None):
        self.config = config
        ...
        if len(device_ids) > 1:
            self.model = torch_geometric.nn.DataParallel(model, device_ids=device_ids)
        ...
        self.checkpoint_dir = os.path.join(cfg_trainer['save_dir'], config['name'], str(config['id']))
        # setup visualization writer instance
        writer_dir = os.path.join(cfg_trainer['log_dir'], config['name'], str(config['id']))
        self.writer = tensorboardX.SummaryWriter(writer_dir)

        # Save configuration file into checkpoint directory:
        if not os.path.exists(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)
        config_save_path = os.path.join(self.checkpoint_dir, 'config_file.json')
    ...
    def _save_checkpoint(self, epoch, emergency=False, save_best=False, num=0):
        """
        Saving checkpoints
        ...
        if not emergency:
            ...
            filename = os.path.join(self.checkpoint_dir, 'checkpoint-curr.pth')
